    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (utilisateur!= null && (utilisateur.getRole() != null && utilisateur.getRole().getNom().equals("admin")) ) {
            ...
            if (id.equals("admin"))
                ...
                retourSuppression = dao.supprimerRole(id) ? "Votre role a été supprimé" : "Il y a eu une erreur";
            ...
            for (Role r : roles) {
                listing +=
                        "            <tr>\n" +
                                "                <th scope='row' class='text-center'>" + r.getNom() + "</th>\n" +
                                "                <th scope='row' class='text-center'>" + dao.recupNbUtilisateursParRole(r.getNom()) + "</th>\n";
                if (!"admin".equals(r.getNom()) && !"user".equals(r.getNom())) {
                    listing += "                <td><button data-toggle='modal' data-target='#modalSuppr' data-whatever = '" + r.getNom() + "' class='btn btn-danger btn-sm ' role='button'><svg width=\"1em\" height=\"1em\" viewBox=\"0 0 16 16\" class=\"bi bi-trash\" fill=\"currentColor\" xmlns=\"http://www.w3.org/2000/svg\">\n" +
